...
일반 PyPI 설치는 새 버전을 하루 한 번 확인하고, 동기화가 대기 상태일 때 자동으로 업데이트한 뒤 재시작합니다.
소스 또는 editable 설치는 작업 파일을 덮지 않도록 자동 업데이트하지 않습니다. 수동 업데이트는 다음 명령을 사용하세요.
...
- iCloud가 업로드 완료를 확인하기 전에는 기준본을 확정하지 않아 이전 클라우드 버전이 로컬 편집을 되돌리지 못합니다.
- 최근 로컬 저장 세대를 재시작 후에도 기억해 iCloud가 직전 저장본을 늦게 재노출해도 충돌로 오인하지 않습니다.
- 콘텐츠 판정은 테스트 가능한 상태 머신으로 분리되고, 세대·삭제 기록은 SQLite journal에 트랜잭션으로 보존됩니다.
- 실제 원격 충돌본은 동기화 Vault 밖의 전용 복구 저장소에 보관해 후속 대량 편집이나 삭제 동기화로부터 격리합니다.
- 기존 문서의 0바이트 전환, 폴더 오인식, 짧은 시간의 대량 삭제는 자동으로 차단하고 오류 상태로 표시합니다.
- 삭제는 유예 시간 뒤 휴지통으로 이동하며, 삭제 기록을 남겨 iCloud가 옛 파일을 다시 노출해도 되살리지 않습니다.
